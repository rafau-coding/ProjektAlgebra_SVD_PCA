...
from sklearn.datasets import fetch_olivetti_faces  # DODANY IMPORT
...
class FaceDetectorPCA:
    ...
    def train(self, face_images):
        print(f"Trening PCA na {len(face_images)} zdjęciach...")
...
def wczytaj_katalog_do_treningu(sciezka_katalogu):
    """
    Zostawione jako opcja. Przeszukuje katalog ze zdjęciami, każde przerabia
    na skalę szarości i standaryzuje do 64x64. Zwraca macierz gotową do włożenia w PCA.
    """
...
if __name__ == "__main__":
    # 1. Zdefiniowanie ścieżek dla testów
    ...
    # 2. Wczytanie i przygotowanie danych do treningu (ZMIANA NA OLIVETTI)
    print("Pobieranie/Wczytywanie bazy Olivetti faces (400 zdjęć)...")
    dataset_olivetti = fetch_olivetti_faces(shuffle=True, random_state=42)
    # dataset_olivetti.data to gotowa macierz, w której każdy wiersz to wektor 4096 (64x64), znormalizowana 0-1.
    twarze_treningowe = dataset_olivetti.data
    ...
    analizuj_moje_zdjecia_i_zapisz_csv(folder_testowy, detector,
                                       nazwa_csv="statystyki_rozpoznawania_twarzy_olivetti.csv")
    ...
    if testy_do_wyswietlenia:
        ...
        plt.suptitle("System Detekcji Twarzy (Model Olivetti): Sprawdzanie zdjęć", fontsize=14, fontweight='bold')
